...
%% Tezos' most powerful feature is its ability to implement protocol capable
...
%% However, as Tezos matures, this return could be gradually lowered to the
...
%% blocks each. Given the one minute block interval, this is about three
...
%% to a specific address for the next cycle. See figure \ref{fig:pos_figure}.
...
%% withholding of the numbers which could be sued to attack the entropy of the seed.
...
%% key. Each interval represents a ``range'' of satoshis.
...
\section{总结}
%% We feel we've built an appealing seed protocol. However, Tezos' true potential
