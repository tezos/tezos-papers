...
\subsubsection{Coins}
There are initially $\num{10000000000}$ (ten billion) coins \change{(the initial extent of the token supply will be the number of tokens issued during the crowdsale and not specifically “10 billion”, an arbitrary placeholder)}, divisible up to two
decimal places \change{(for the sake of precision we will in actuality be using eight digits after the decimal)}. We suggest that a single coin be referred to as a ``tez''
...
Bonds are one year \change {(bonds will now only last a single cycle, given the high opportunity cost and little benefit to security of a long bond)} security deposits purchased by miners \change {(endorsers will also be required to purchase bonds)}.
...
are destroyed along with the coins they contain \change {(inactive addresses will no longer lose their funds after one year as initially proposed in the white paper, they will only lose their staking rights until they become active again. What it means is that if an address is inactive, it will not be selected to create blocks (which would slow down the consensus algorithm), and it will not be allowed to vote until it is reactivated (to avoid uncertainty about participation rate). This also affects governance)}.
...
forfeited in the event of a double mining or double signing \change {{it appears you've already changed this, as the Medium article states that signers, assuming this is the same as 'endorsers', were not originally but now are required to bond)}.
